...
exports.register = async (req, res) => {
  try {
    ...
    sendConfimationEmail(
      ...
      email,
      `${process.env.SERVER_URL}/api/users/confirmation/${userData._id}/${emailConfirmToken}`
    );
 ...
 */
exports.generateConfirmationToken = async (req, res) => {
  try {
    ...
    // Check for already confirmed email
    if (user.emailConfirm) {
      return res.status(200).json({ msg: "Your Email is already confirmed" });
    }
    ...
    if (
      ...
      return res
        .status(200)
        .json({ msg: "A Confirmation email is already sent to your email" });
    }

    const token = crypto.randomBytes(20).toString("hex");
    user.emailConfirmToken = token;
    user.emailConfirmExpires = Date.now() + 365 * 24 * 60 * 60 * 1000; // 1 year
...
exports.confirmEmail = async (req, res) => {
  try {
    ...
    const tokenExpiryTime = new Date(user.emailConfirmExpires);

    if (currentTime > tokenExpiryTime) {
      return res.status(400).json({ msg: "Your link has expired" });
    }

    user.emailConfirm = true;
    user.emailConfirmToken = null;

    // Update user status based on phone verification
    if (user.phone) {
      user.status = "active";
    } else {
...
exports.resetPasswordEmail = async (req, res) => {
  try {
    ...
    res.render("reset-email-template", {
      ...
      resetLink: `${process.env.FRONTEND_URL}/reset-password/${user._id}`,
    });
  } catch (err) {
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    const user = await User.findById(userId)
      .populate("ownedPets", "title images species category slug")
      .populate("purchasedPets", "title images species category slug");
  ...
  try {
    ...
    const analytics = await Promise.all(
      referrals.map(async (referral) => {
        const coinsSpent = referral.coins || 0;
        const earnings = coinsSpent * 0.1; // 10% of coins spent
        return {
          userId: referral._id,
          userEmail: referral.email,
          userName: `${referral.firstname} ${referral.lastname}`,
          coinsSpent,
          earnings,
        };
      })
    ...
    res.status(200).json({
      ...
      totalEarnings: analytics.reduce((sum, item) => sum + item.earnings, 0),
    });
  } catch (err) {

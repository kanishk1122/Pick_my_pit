...
/**
 * Handle admin login with multiple authentication strategies
 */
exports.loginAdmin = async (req, res) => {
  try {
    ...
    console.log(
      `Found user: ${foundUser.email} (Model: ${userModel || userCollection})`
    );
...
/**
 * Run system diagnostics (only in development)
 */
...
exports.createAdmin = async (req, res) => {
  try {
    ...
    // If no admin model found, we can't create a new admin
    if (!adminModel) {
      return res.status(500).json({
        success: false,
        msg: "No admin model available to create admin",
      });
    }
